...
func GetSalads(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			web.ErrorResponse(w, fmt.Errorf("%s: преобразование минимального рейтинга к float: %w", prompt, err).Error(), http.StatusBadRequest)
			return
		}
...
func GetSaladsWithStatus(app *app.App) http.HandlerFunc { // FIXME
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if page == "" {
			web.ErrorResponse(w, fmt.Errorf("%s: пустой номер статуса", prompt).Error(), http.StatusBadRequest)
			return
		}
		statusInt, err := strconv.Atoi(status)
		if err != nil {
			web.ErrorResponse(w, fmt.Errorf("%s: преобразование номера статуса к int: %w", prompt, err).Error(), http.StatusBadRequest)
			return
		}
...
func GetUserComment(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if userId == "" {
			web.ErrorResponse(w, fmt.Errorf("%s: пустой id пользователя", prompt).Error(), http.StatusBadRequest)
			return
		}
		userUuid, err := uuid.Parse(userId)
		if err != nil {
			web.ErrorResponse(w, fmt.Errorf("%s: преобразование id пользователя к uuid: %w", prompt, err).Error(), http.StatusBadRequest)
			return
		}
...
func DeleteComment(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if comment.AuthorID != userUuid { // TODO: mb admin can delete every comment
			web.ErrorResponse(w, fmt.Errorf("%s: только автор комментария может удалить его", prompt).Error(), http.StatusBadRequest)
			return
		}
...
func UpdateComment(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if commentDb.AuthorID != userUuid {
			web.ErrorResponse(w, fmt.Errorf("%s: только автор комментария может изменить его", prompt).Error(), http.StatusBadRequest)
			return
		}
...
func UpdateSalad(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid { // TODO: mb admin can change every salad?
			web.ErrorResponse(w, fmt.Errorf("%s: только автор салата может изменить его", prompt).Error(), http.StatusBadRequest)
			return
		}
		...
		web.SuccessResponse(w, http.StatusOK, map[string]interface{}{
			"salad": web.ToSaladTransport(saladDb),
		})
	}
}
...
func CreateRecipe(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid {
			web.ErrorResponse(w, fmt.Errorf("%s: только автор салата может создать рецепт", prompt).Error(), http.StatusBadRequest)
			return
		}
...
func UpdateRecipe(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid && userRole != "admin" { // TODO: mb admin can change every recipe?
			web.ErrorResponse(w, fmt.Errorf("%s: только автор рецепта может изменить его", prompt).Error(), http.StatusBadRequest)
			return
		}
		...
		web.SuccessResponse(w, http.StatusOK, map[string]interface{}{
			"recipe": web.ToRecipeTransport(recipeDb),
		})
	}
}
...
func LinkTypeToSalad(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid {
			web.ErrorResponse(w, fmt.Errorf("%s: только автор салата может добавлять типы к салату", prompt).Error(), http.StatusBadRequest)
			return
		}
...
func UnlinkTypeFromSalad(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid {
			web.ErrorResponse(w, fmt.Errorf("%s: только автор салата может изменять типы салата", prompt).Error(), http.StatusBadRequest)
			return
		}
...
func LinkIngredientToSalad(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid {
			web.ErrorResponse(w, fmt.Errorf("%s: только автор салата может добавлять ингредиенты к салату", prompt).Error(), http.StatusBadRequest)
			return
		}
...
func UnlinkIngredientFromSalad(app *app.App) http.HandlerFunc { // FIXME
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid {
			web.ErrorResponse(w, fmt.Errorf("%s: только автор салата может удалять ингредиенты салата", prompt).Error(), http.StatusBadRequest)
			return
		}
...
func DeleteRecipeStep(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid {
			web.ErrorResponse(w, fmt.Errorf("%s: только автор рецепта может удалять шаги", prompt).Error(), http.StatusBadRequest)
			return
		}
...
func UpdateRecipeStep(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if saladDb.AuthorID != userUuid {
			web.ErrorResponse(w, fmt.Errorf("%s: только автор рецепта может изменять шаги", prompt).Error(), http.StatusBadRequest)
			return
		}
